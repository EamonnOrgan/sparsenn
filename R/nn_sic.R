...
#' @param method One of c("single", "group")
...
#' @export
nn_sic <- function(X, y, q, method = "single",
                   grouping = NULL,
                   eps_start = 10, t = 100, r = 0.87, n_iter = 1,
                   n_init = 1, tanh_param = FALSE, c = NULL) {
  ...
  if (method[1] == "single") {
    nn_logl <- nn_logl_sic
  } else if (method[1] == "group" & is.null(grouping)) {
    stop("Error: grouping must not be NULL when method == group")
...
#' Neural network SIC with twostep option
#'
#' @param X Input data
#' @param y Response vector
#' @param q Number of hidden nodes
...
#' @return Function value
#' @export
nn_sic_twostep <- function(X, y, q, method = c("single", "group", "twostep"),
